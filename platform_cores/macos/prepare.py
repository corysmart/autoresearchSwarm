...
def train_tokenizer():
    ...
    finished = time.time()
    print(f"Tokenizer: trained in {finished - started:.1f}s, saved to {tokenizer_pkl}")
    ...
    while True:
        for filepath in parquet_paths:
            ...
            for row_group_index in range(parquet_file.num_row_groups):
                ...
                for index in range(0, len(batch), tokenizer_batch_size):
                    yield batch[index:index + tokenizer_batch_size], epoch
        epoch += 1
