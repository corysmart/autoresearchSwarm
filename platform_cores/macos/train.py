"""
Autoresearch pretraining script. Single-platform macOS variant derived from
miolini/autoresearch-macos, with harness checkpoint hooks preserved.
...
class GPT(nn.Module):
    ...
    def estimate_flops(self):
        ...
        nparams = sum(parameter.numel() for parameter in self.parameters())
        value_embeds_numel = sum(value_embed.weight.numel() for value_embed in self.value_embeds.values())
        nparams_exclude = (
    ...
    def num_scaling_params(self):
        wte = sum(parameter.numel() for parameter in self.transformer.wte.parameters())
        value_embeds = sum(parameter.numel() for parameter in self.value_embeds.parameters())
        lm_head = sum(parameter.numel() for parameter in self.lm_head.parameters())
        transformer_matrices = sum(parameter.numel() for parameter in self.transformer.h.parameters())
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        for shape in sorted({parameter.shape for parameter in matrix_params}):
            group_params = [parameter for parameter in matrix_params if parameter.shape == shape]
...
# ---------------------------------------------------------------------------
# Optimizer (MuonAdamW, platform-aware)
# ---------------------------------------------------------------------------
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_muon(self, group):
        ...
        stacked_grads = torch.stack([parameter.grad for parameter in params])
...
model = torch.compile(model_core, dynamic=False) if device_type == "cuda" else model_core
...
def sync_device(current_device_type):
    if current_device_type == "cuda":
        torch.cuda.synchronize()
    elif current_device_type == "mps":
